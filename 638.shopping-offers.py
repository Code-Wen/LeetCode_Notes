...
class Solution:
    def shoppingOffers(self, price: List[int], special: List[List[int]], needs: List[int]) -> int:
        ...
        # Use a dictionary to cache 
        history = {}
        
        def helper(needs):
            if tuple(needs) in history:
                return history[tuple(needs)]
            n = len(price)
            min_cost = dotProduct(price, needs)
            for offer in special:
                if all([offer[i] <= needs[i] for i in range(n)]):
                    modified_needs = [needs[i]-offer[i] for i in range(n)]
                    cost = offer[-1] + helper(modified_needs)
                    min_cost = min(min_cost, cost)
            history[tuple(needs)] = min_cost
            return min_cost
