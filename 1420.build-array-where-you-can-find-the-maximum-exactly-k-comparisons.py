...
class Solution:
    def numOfArrays(self, n: int, m: int, k: int) -> int:
        """
        dp[i][j][k] means how many possibilities with the restriction:
        ...
        for i, j, s in itertools.product(range(1, n + 1), range(1, k + 1), range(m + 1)):              
            dp[i][j][s] += (dp[i - 1][j][s] * s) % mod  # no new jump
            dp[i][j][s] += sum(dp[i - 1][j - 1][1:s]) % mod # one more jump
