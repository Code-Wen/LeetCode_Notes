...
class Solution:
    def isRationalEqual(self, S: str, T: str) -> bool:
        def longVersion(s):
            ...
            integer, non_int = s.split('.')
            idx = non_int.find('(')
            if idx >= 0:
                non_repeat = non_int[:idx]
                repeat =  non_int[idx+1:-1]
            else:
                non_repeat = non_int+'0'
                repeat = ''
            if repeat and all([repeat[i]=='9' for i in range(len(repeat))]):
                if non_repeat == '':
                    integer = str(int(integer)+1)
                    non_int = '0'
                else:
                    add = (int('1'+non_repeat)+1)//(10**len(non_repeat))-1
                    integer = str(int(integer)+add)
                    non_int = str(int('1'+non_repeat)+1)[1:]
            else:
                non_int = (non_repeat + repeat*30)[:16]
            num = float(integer+'.'+non_int)
            return num

        return longVersion(S) == longVersion(T)
