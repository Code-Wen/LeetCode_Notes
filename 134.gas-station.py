...
class Solution:
    def canCompleteCircuit(self, gas: List[int], cost: List[int]) -> int:
        total, start, n = 0, 0, len(gas)
        res = -1
        while start < n:
            remaining = gas[start] - cost[start]
            # Once find a good starting point, see how far we can go from there
            if remaining >= 0:
                res = start  # record the starting location
                i = start + 1
                while i < n and remaining + gas[i]-cost[i] >= 0:
