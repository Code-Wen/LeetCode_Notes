...
class Solution:
    def maxSubarraySumCircular(self, A: List[int]) -> int:
        # Use Kadane's algorithm to find the max and min of (possibly empty) subarrays sum
        curMax, curMin, resMax, resMin = 0, 0, 0, 0
        for n in A:
            curMax = max(n, curMax+n)
            curMin = min(n, curMin+n)
            resMax = max(resMax, curMax)
            resMin = min(resMin, curMin)
        # If max subarray sum == 0: all numbers in A is non-positive
        if resMax == 0:
            return max(A)
        # If max subarray sum > 0: at least 1 positive in A
        else: 
            return max(resMax, sum(A)-resMin)
# @lc code=end
