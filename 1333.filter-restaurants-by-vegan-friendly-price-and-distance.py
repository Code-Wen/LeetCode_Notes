...
class Solution:
    def filterRestaurants(self, restaurants: List[List[int]], veganFriendly: int, maxPrice: int, maxDistance: int) -> List[int]:
        def veganCheck(criterion, vegan):
            if criterion == 0:
                return True
            else:
                return vegan == 1
        
        return [y[1] for y in sorted([(x[1], x[0]) for x in restaurants if veganCheck(veganFriendly, x[2]) and maxPrice >= x[3] and maxDistance >= x[4]])[::-1]]
