...
class Solution:
    def minReorder(self, n: int, connections: List[List[int]]) -> int:
        directedEdges, undirectedEdges = collections.defaultdict(set), collections.defaultdict(set)
        for start, end in connections:
            directedEdges[start].add(end)
            undirectedEdges[start].add(end)
            undirectedEdges[end].add(start)
        q, seen, res = collections.deque([0]), {0}, 0
        while q:
            dest = q.popleft()
            for k in undirectedEdges[dest]:
                if k in seen:
                    continue
                else:
                    if dest not in directedEdges[k]:
                        res += 1
                    q.append(k)
                    seen.add(k)
        return res
