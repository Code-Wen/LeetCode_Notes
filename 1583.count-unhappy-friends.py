...
class Solution:
    def unhappyFriends(self, n: int, preferences: List[List[int]], pairs: List[List[int]]) -> int:
        ...
        res = 0
        def checkUnhappiness(i):
            """
            Return 1 if i is not happy, 0 otherwise.
            """
            i_partner_pos = preferences[i].index(pairs_dic[i])
            for k in preferences[i][:i_partner_pos]:
                i_pos = preferences[k].index(i)
                partner_pos = preferences[k].index(pairs_dic[k])
                if i_pos < partner_pos:
                    return 1
            return 0
        for i in range(n):
            res += checkUnhappiness(i)
        return res
